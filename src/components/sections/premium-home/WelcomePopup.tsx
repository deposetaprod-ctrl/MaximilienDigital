...
import { CheckCircle2, Wand2, MessageCircle, GraduationCap, Star } from "lucide-react";
...
export function WelcomePopup() {
  ...
  useEffect(() => {
    ...
    fetch("/api/track-funnel", {
      ...
      body: JSON.stringify({ step: "page_view", sessionId: id, action: "Ouverture Welcome Popup" }),
    }).catch(console.error);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch("/api/track-funnel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ step: "completed", sessionId, action: `Welcome Popup Soumis: ${flowAction}` }),
      });
  ...
  return (
    <div className="dialog-overlay" data-clarity-unmask="true" style={{ backdropFilter: "blur(12px)", backgroundColor: "rgba(16, 13, 11, 0.85)" }}>
      {/* We purposefully don't close on overlay click to encourage completion, but the cross is there */}
      <div className="dialog-content" onClick={(e) => e.stopPropagation()} style={{ border: "1px solid rgba(255, 191, 63, 0.15)", background: "rgba(20, 15, 12, 0.95)" }}>
        ...
        <button 
          ...
          aria-label="Fermer" 
          onClick={closeDialog}
          style={{ position: "absolute", right: "20px", top: "20px", background: "transparent", border: "none", color: "rgba(255,255,255,0.3)", fontSize: "24px", cursor: "pointer", outline: "none", zIndex: 10 }}
        >
          ×
        </button>
      ...
      {!isSuccess ? (
        <>
          <form onSubmit={(e) => { e.preventDefault(); if (step === 2 && flowAction !== "freelance") handleSubmit(); else if (step === 3 && flowAction === "freelance") handleSubmit(); else setStep(s => s + 1); }} noValidate>
            
            {/* STEP 0 : Résumé du projet */}
            {step === 0 && (
            ...
            {/* STEP 2 : Informations complémentaires et Email (OU Question de confiance si Freelance) */}
            {step === 2 && (
              <fieldset style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}>
                {flowAction === "maquette" && (
                ...
                {flowAction === "freelance" && (
                  <div style={{ textAlign: "center", marginBottom: "15px" }}>
                    ...
                    <div style={{ position: 'relative', display: 'inline-block', marginBottom: '16px' }}>
                      ...
                      />
                      <span style={{ position: 'absolute', bottom: '4px', right: '4px', width: '18px', height: '18px', backgroundColor: 'var(--lime)', border: '3px solid #1a1412', borderRadius: '50%', boxShadow: '0 0 10px rgba(189, 255, 0, 0.5)' }}></span>
                    </div>
            ...
            {/* STEP 3 : Freelance form handling (Oui/Non/Peut-etre) */}
